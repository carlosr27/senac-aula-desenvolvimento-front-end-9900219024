...
function montarFilme() {
  ...
  $(".filme-background .filme-nome").text(filme.nome);
  ...
  const destaque = $(".filme-estrelas span").clone();
  $(".filme-estrelas")
    .html(`&nbsp;${filme.infos.estrelando.join(", ")}`)
    .prepend(destaque);
  const destaqueCriacao = $(".filme-realizacao span").clone();
  $(".filme-realizacao")
    .html(`&nbsp;${filme.infos.criacao.join(", ")}`)
  ...
  if (adicionado) {
    ...
    $("button.js-adicionar").attr("data-adicionado", adicionado);
  }
